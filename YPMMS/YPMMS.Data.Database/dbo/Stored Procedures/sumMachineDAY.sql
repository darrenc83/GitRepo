...
CREATE PROCEDURE [dbo].[sumMachineDAY]
  @year int,
  @month int,
  @day int,
  @Acc money OUTPUT
...
FETCH NEXT FROM db_cursor INTO @machineid  
	...
	-- change  this [dbo].[MachineValues] to  the name of the table i think its called LiveMachineIncome
      set @temp = ( select top 1 Accum from [dbo].[MachineLevelIncome]  where (machineid=@machineid and [nYear]=@year and [nMonth]=@month and [nDay]=@day) order by [Timestamp] desc  ) 
       if ( len(@temp) >0  )
		  begin
			set @value += @temp
	 	  end 
